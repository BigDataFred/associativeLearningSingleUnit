...
for curPat = 1:length(pId)
    for curExp = 1:length(expMode)
        for curMode = 1:length(spkMode)
            %%
            tmp = dir([rdsPath,pId{curPat},'_',expMode{curExp},'_*_lfpDataStimLockedSegmenteddownsampled.mat']);
            ...
            if ~isempty(tmp)
                ...
                for curSesh = 1:length(sesh)
                    ...
                    %%
                    fNspk2LFP = [pId{curPat},'_',expMode{curExp},'_',sesh{curSesh},'_spk2LFPCoupling',spkMode{curMode},'_plv_lowFreq.mat'];
                    fNspk2LFP = dir([p2d2,fNspk2LFP]);
                    [ plvSigDat ] = load([p2d2,fNspk2LFP.name]);                                                                             
                    
                    %%
                    load([savepath,pId{curPat},'_',expMode{curExp},'_',sesh{curSesh},'_spkParams_',spkMode{curMode},'.mat']);
